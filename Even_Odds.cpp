...
signed main(){
    ...
    // from 1 to n+1/2 terms will be surely odd term1=k-(n+1)/2 // ans1=2*term1-1
    // rest will be even  any term in even : term=2 (n-(n+1)/2 ) // ans2 = term2*2

    int mid = (n+1)/2, ans;

    if(k<=mid){
        // odd
        int term1=k; ans=2*term1-1;
        cout<<ans;

    }else{
        //even
        int term2=k-mid; ans=term2*2; 
        cout<<ans;
    }
